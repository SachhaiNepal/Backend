...
class RegisterFollower(APIView):
    ...
    def post(request):
        ...
        if serializer.is_valid():
            ...
            new_follower.profile.country = Country.objects.get(id=validated_data["country"])
            new_follower.profile.province = Province.objects.get(id=validated_data["province"])
            new_follower.profile.district = District.objects.get(id=validated_data["district"])
