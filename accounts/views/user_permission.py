...
from accounts.serializers.permissions import (AssignPermissionSerializer,
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if not user.has_perm(get_permission_string(permission)):
                ...
                return Response(
                    {"detail": "User permission added", "user": user_serializer.data},
    ...
    def delete(self, request):
        ...
        if serializer.is_valid():
            ...
            if user.has_perm(get_permission_string(permission)):
                ...
                return Response(
                    {"detail": "User permission removed", "user": user_serializer.data},
                    status=status.HTTP_200_OK,
                )
            else:
                return Response(
...
class UserPermissionDetail(APIView):
    ...
    def get(self, request, pk):
        ...
        return Response(
            {"models": len(grouped_permissions), "data": grouped_permissions},
            status=status.HTTP_200_OK,
        )
